...
export default function BookingScreen() {
  ...
  // --- NUEVA FUNCIÓN INTELIGENTE PARA FILTRAR HORAS PASADAS ---
  const getAvailableTimeSlots = () => {
    ...
    // Si el usuario eligió HOY, filtramos las horas que ya pasaron
    ...
    // Si eligió un día de mañana en adelante, mostramos todas las horas
    return TIME_SLOTS;
  };

  // --- OBTENER FECHA MÍNIMA (HOY) PARA EL CALENDARIO ---
      ...
      <div className="w-full max-w-md flex flex-col h-full min-h-[90vh]">
        ...
        <div className="flex-1">
          {step === 1 && (
            <div className="flex flex-col gap-4">
              {DUMMY_BARBERS.map((barber) => (
                <button key={barber.id} onClick={() => { setSelectedBarber(barber); setStep(2); }} className="bg-barbas-dark border border-white/5 hover:border-barbas-gold rounded-xl p-4 flex items-center text-left">
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4">
              {DUMMY_SERVICES.map((service) => (
                <button key={service.id} onClick={() => { setSelectedService(service); setStep(3); }} className="bg-barbas-dark border border-white/5 hover:border-barbas-gold rounded-xl p-4 flex justify-between items-center text-left">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div>
                ...
                <input 
                  ...
                  min={getTodayString()} // Bloquea días anteriores correctamente
              ...
              {selectedDate && (
                <div>
                  ...
                  {availableTimeSlots.length === 0 ? (
                    <p className="text-red-400 text-sm text-center mt-4 border border-red-500/30 p-3 rounded-lg">
                      Ya no hay turnos disponibles para hoy. Por favor elige otro día.
                    </p>
                  ) : (
          ...
          {step === 4 && (
            <div className="flex flex-col items-center justify-center h-full text-center mt-20">
              ...
              <button onClick={() => router.push("/home")} className="bg-barbas-gold text-barbas-black font-bold py-3 px-8 rounded-lg hover:bg-yellow-500 w-full">VOLVER AL INICIO</button>
            </div>
          )}
        </div>
        ...
        {step === 3 && (
          <button
            ...
            className="w-full bg-barbas-gold text-barbas-black font-bold text-lg py-4 rounded-xl mt-6 hover:bg-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
